import { boolResponseSchema, idResponseSchema, messageResponseSchema, } from '../../schemas/shared.js';
import { ballotResponseSchema, electionConfigPayloadSchema, electionHashSchema, electionInfoResponseSchema, voteCountResponseSchema, voteDataSchema, voterIdSchema, } from '../../schemas/elections.js';
import { activateElection, callElection, createElection, deleteElection, getBallot, getElectionInfo, getHasVoted, getVoteCount, vote, } from '../../controllers/elections.js';
...
const elections = async (fastify, opts) => {
    fastify.route({
        method: 'PUT',
        url: '/elections',
    ...
    fastify.route({
        ...
        handler: async (request, reply) => {
            ...
            const [value, error] = await getHasVoted(electionHash, voterIdSchema.parse(Number(voterId)));
            if (error != null) {
                handleError(reply, error);
                return;
            }
    ...
    fastify.route({
        ...
        preHandler: [fastify.authenticate],
        handler: async (request, reply) => {
            const [value, error] = await activateElection(request.params.electionHash);
            if (error != null) {
                handleError(reply, error);
                return;
            }
    ...
    fastify.route({
        ...
        handler: async (request, reply) => {
            const [voteCount, error] = await getVoteCount(request.params.electionHash);
            if (error != null) {
                handleError(reply, error);
                return;
            }
    ...
    fastify.route({
        ...
        handler: async (request, reply) => {
            const user = userTokenPayloadSchema.parse(request.user);
            const electionHash = electionHashSchema.parse(request.params.electionHash);
