...
export default fp(async (fastify) => {
    ...
    fastifyPassport.default.use('google', new GoogleStrategy({
    ...
    fastify.get('/api/auth/google/callback', {
        ...
        const tokens = await fastify.getTokens(userTokenPayloadSchema.parse(request.user));
        reply.setTokens(tokens).redirect('http://localhost:5173/home');
    });
    fastify.get('/api/login', fastifyPassport.default.authenticate('google', {
        scope: ['profile', 'email'],
    }));
